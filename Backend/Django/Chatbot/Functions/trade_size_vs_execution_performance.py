...
def trade_size_vs_execution_performance(file_id, start_date=None, end_date=None):
    ...
    trades_df['trade_timestamp'] = pd.to_datetime(trades_df['trade_timestamp']).astype(int) / 10**9
    market_prices_df['trade_timestamp'] = pd.to_datetime(market_prices_df['trade_timestamp']).astype(int) / 10**9
    ...
    labels = ['1,000 - 5,000', '5,001 - 10,000',
              '10,001 - 50,000', '50,001 - 100,000', '100,001+']
    merged_df['Trade Size Range (Shares)'] = pd.cut(
        merged_df['trade_size'], bins=bins, labels=labels, right=False)
    ...
    merged_df['Price Deviation (%)'] = 100 * abs(merged_df['trade_price'] - merged_df.get(
        'trade_price_market', 0)) / merged_df.get('trade_price_market', 1)
    ...
    # Simulate trade completeness
    completeness_threshold = 0.01  # 1%
    merged_df['Trade Completeness (%)'] = 100 * (
        abs(merged_df['Price Impact (bps)']) <= 100 * completeness_threshold)
    # End simulating trade completeness

    


    results_df = merged_df.groupby('Trade Size Range (Shares)').agg(
        Average_Execution_Price=('Average Execution Price', 'mean'),
        Price_Impact_bps=('Price Impact (bps)', 'mean'),
        Price_Deviation_Percent=('Price Deviation (%)', 'mean'),
