...
def price_impact_trade_completeness(file_id, start_date=None, end_date=None):
    ...
    for result in results:
        if not pd.isna(result['Price_Impact_bps']):
            final_result = [
                result['Trade Size Range (Shares)'],
                round(result['Price_Impact_bps'], 2),
                # result['Trade_Completeness']
            ]
            final_results.append(final_result)

    chart = build_chart(final_results, "bar")
