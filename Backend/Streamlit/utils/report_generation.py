...
pdfmetrics.registerFont(TTFont('CraftworkGrotesk-Bold', 'utils/fonts/CraftworkGrotesk-Bold.ttf'))
pdfmetrics.registerFont(TTFont('CraftworkGrotesk-Regular', 'utils/fonts/CraftworkGrotesk-Regular.ttf'))
pdfmetrics.registerFont(TTFont('CraftworkGrotesk-SemiBold', 'utils/fonts/CraftworkGrotesk-SemiBold.ttf'))
pdfmetrics.registerFont(TTFont('CraftworkGrotesk-Medium', 'utils/fonts/CraftworkGrotesk-Medium.ttf'))
...
def generate_report(report_calculation):
    try:
        ...
        pdf_content = buffer.getvalue()  # Get the PDF content

        buffer.close()  # Close the buffer
        
        return pdf_content  # Return the PDF content directly

    except Exception as e:
        print(f'Report generation found error {e}')
        return {'error': str(e)}
...
def create_report_content(report_calculation, styles, doc):
    ...
    Average_Optimal_Strategy =report_calculation['potential_savings']['total_savings']

    table_data = report_calculation['most_slippage_trades']

    backtesting_data = report_calculation['potential_savings']['potential_saving_table']
    ...
    data = [
        ...
        [Paragraph('Your <img src="utils/images/hoodwinked-logo-pdf.png" width="277" height="53"></img>', title_style)],
    ...
    # add image here
    img_path = 'utils/images/pdf-first-image.png'  # Replace with actual path to your image
    ...
    # add these two images in the table report_calculation['avg_slippage_bar_bytes'], report_calculation['xs_returns_line_bytes']
    img_width, img_height = 450, 250

    if isinstance(report_calculation['avg_slippage_bar_bytes'], BytesIO):
        img_with_box = ImageWithBox2(report_calculation['avg_slippage_bar_bytes'], img_width, img_height)
        elements.append(img_with_box)
    else:
        print('Skipping image append: avg_slippage_bar_bytes is not a BytesIO object')

    
    elements.append(Spacer(1, 0.2*inch))

    if isinstance(report_calculation['xs_returns_line_bytes'], BytesIO):
        img_with_box = ImageWithBox2(report_calculation['xs_returns_line_bytes'], img_width, img_height)
        elements.append(img_with_box)
    else:
        print('Skipping image append: xs_returns_line_bytes is not a BytesIO object')
    ...
    table = Table(data,rowHeights=[60,50,50,50,50,50],colWidths=[doc.width * .167, doc.width * .167, doc.width * .167, doc.width * .167, doc.width * .167, doc.width * .167])  
    table.setStyle(TableStyle([
        ('BOX', (0, 0), (-1, -1), 1, colors.black),  # Add a black border around the whole table
    ...
    data = [
        [
            ...
            Paragraph('Action', table_header_small),
            Paragraph('Symbol', table_header_small),
            Paragraph('Your Price', table_header_small),
    ...
    for row in table_data:
        ...
        data.append([Paragraph(str(cell), normal_style) for cell in row_data])  # Fill cells with data
    # Create the table with the rows in a 5 columns
    table = Table(data, rowHeights=[None] * len(data), colWidths=[doc.width * .130] * 7 + [doc.width * .2])  
    table.setStyle(TableStyle([
        ('BOX', (0, 0), (-1, -1), 1, colors.black),  # Add a black border around the whole table
        ('LINEBEFORE', (1, 0), (-1, -1), 1, colors.black),  # Add a black border around the whole table
        ('LINEBELOW', (0, 0), (-1, 0), 1, colors.black),  # Add a black border around the whole table
