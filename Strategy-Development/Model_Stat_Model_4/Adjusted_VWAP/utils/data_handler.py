...
from utils.fetch_merge_data import fetch_and_merge_data
...
class InferenceDataHandler():
    ...
    def create_data_w_forecasts(self, data, steps = 390):
        ...
        def generate_trading_hours_timestamps(start_timestamp, forecast_steps):
            """
            Generate a series of future timestamps within trading hours, accounting for weekends, holidays, and DST.

            Args:
                start_timestamp (pd.Timestamp): The last timestamp in the dataset, in UTC.
                forecast_steps (int): Number of timestamps to generate.

            Returns:
                list: List of UTC timestamps within trading hours.
            """
            # Initialize the holiday calendar and define a custom business day
            ...
            # Start from the provided timestamp, converted to Eastern Time
            start_timestamp = pd.Timestamp(start_timestamp, tz='UTC').astimezone(eastern)
            timestamps = []
            current_time = start_timestamp

            while len(timestamps) < forecast_steps:

                trading_start_hour = 9  # Eastern Time
                trading_start_minute = 30
                trading_end_hour = 16   # Eastern Time

                # Define trading hours in Eastern Time
                trading_start = current_time.replace(hour=trading_start_hour, minute=trading_start_minute, second=0, microsecond=0)
                trading_end = current_time.replace(hour=trading_end_hour, minute=0, second=0, microsecond=0)

                # Check if the day is a valid trading day
                if current_time.date() in holidays.date or current_time.weekday() >= 5:
                    # Move to the next business day
                    current_time = (current_time + custom_bday).normalize().astimezone(eastern)
                    continue

                # Start generating timestamps within trading hours
                if current_time < trading_start:
                    current_time = trading_start  # Start at the beginning of trading hours if before it

                while current_time <= trading_end and len(timestamps) < forecast_steps:
                    # Append the timestamp in UTC
                    timestamps.append(current_time.astimezone(pytz.UTC))
                    current_time += pd.Timedelta(minutes=1)

                # Move to the next business day after trading hours end
        ...
        new_data = new_data[1:] # Because the first row is the last timestamp (don't need that)
