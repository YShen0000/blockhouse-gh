...
class TradingEnvironment(gym.Env):
    def __init__(self,
        ...
        self.cumulative_penalty = 0 # Total reward in an episode.
        ...
        self.remaining_inventory_log = [] # Amount of inventory remaining at of episodes.
        ...
        self.action_space = spaces.Box(low=-np.inf, high=0, dtype=np.float32)
        # Extra state columns:
    ...
    def step(self, action):
        ...
        if done:
            self.penalty_trajectory.append(self.cumulative_penalty/self.actions_ct)
            self.current_step = self.current_step//self.reset_count + 1
            self.current_step = self.current_step * self.reset_count
            self.remaining_inventory_log.append(self.remaining_inventory)
    ...
    def _calculate_reward(self, execution_price, action):
        ...
        reward = np.exp(-penalty - (2 * kappa * (penalty ** 2))) + np.log(action/1.66+0.4)
    ...
    def render(self):
        print('--------------------------------------------------')
        if self.actions_ct:
            print(f'Penalty per trade: {self.cumulative_penalty/self.actions_ct}')
