...
PPO_MODEL_BEST_PARAMS_TAB = {'learning_rate': 0.0179457, 'n_steps': 128, 'batch_size': 128,
                        'gamma': 0.951791, 'clip_range': 0.128042, 'n_epochs': 8, 'ent_coef': 0.0002526, 'verbose': 0}
...
class CustomNetwork(nn.Module):
    ...
    def __init__(
        self,
        observation_space, continuous_cols, scenario, market_cap,
        feature_dim: int = 256,
        ...
        self.policy_tab_transformer = TabTransformerExtractor(observation_space, continuous_cols, scenario, market_cap, features_dim = feature_dim)

        self.value_tab_transformer = TabTransformerExtractor(observation_space, continuous_cols, scenario, market_cap, features_dim = feature_dim)

        # Policy network
        self.policy_net = nn.Sequential(self.policy_tab_transformer, nn.Linear(feature_dim, last_layer_dim_pi), nn.Tanh())
        # Value network
        self.value_net = nn.Sequential(self.value_tab_transformer, nn.Linear(feature_dim, last_layer_dim_vf))
...
class TabActorCriticPolicy(ActorCriticPolicy):
    def __init__(
        self,
    ...
    def __repr__(self):
        return f"TabActorCriticPolicy(continuous_cols={self.continuous_cols}, scenario={self.scenario}, market_cap={self.market_cap})"
