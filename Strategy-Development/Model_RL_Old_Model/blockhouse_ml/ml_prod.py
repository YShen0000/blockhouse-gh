...
def run_equities_inference(ticker, start_timestamp = (datetime.now(pytz.UTC) - timedelta(days=2)).strftime('%Y-%m-%d'), end_timestamp=datetime.now(pytz.UTC).strftime('%Y-%m-%d'), timeframe=90, inventory=10000, trade_set_counter=1, action = 'buy'):
    if action == 'sell':
        return equities_sell_inference.run_pipeline(ticker, start_timestamp = start_timestamp, end_timestamp=end_timestamp, timeframe=timeframe, inventory=inventory, trade_set_counter=trade_set_counter)
