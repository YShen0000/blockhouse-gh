...
def run_equities_sell_inference(ticker, day_of_backtest, timeframe=390, inventory=10000, data=None, trade_set_counter=1):
    ...
    # Merge trades with data based on datetime
    merged_trades = pd.merge_asof(trades, data[['datetime', 'bid_price']], 
                                  left_on='timestamp', right_on='datetime', 
                                  direction='nearest')

    # Replace the 'price' column with 'bid_price'
    merged_trades['price'] = merged_trades['bid_price']

    # Drop the 'bid_price' and 'datetime' columns as they're no longer needed
    merged_trades = merged_trades.drop(columns=['bid_price', 'datetime'])

    print("Modified trades:", merged_trades)
...
def run_equities_buy_inference(ticker, day_of_backtest, timeframe=390, inventory=10000, data=None, trade_set_counter=1):
    ...
    # Merge trades with data based on datetime
    merged_trades = pd.merge_asof(trades, data[['datetime', 'ask_price']], 
                                  left_on='timestamp', right_on='datetime', 
                                  direction='nearest')

    # Replace the 'price' column with 'ask_price'
    merged_trades['price'] = merged_trades['ask_price']

    # Drop the 'ask_price' and 'datetime' columns as they're no longer needed
    merged_trades = merged_trades.drop(columns=['ask_price', 'datetime'])
