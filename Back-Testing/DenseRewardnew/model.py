...
from test_model.fetch_merge_data import PolygonClient
...
def infer_model(side='buy', timeframe=390, inventory = 1000, ticker = 'MSFT', end_timestamp = datetime.now(pytz.UTC).strftime('%Y-%m-%d'), backtest=False):
    ...
    model.df.to_csv('data_with_indicators.csv', index=False)
...
print(infer_model(side='buy',inventory = 4000, ticker='TSLA', end_timestamp='2024-09-09', backtest=True))