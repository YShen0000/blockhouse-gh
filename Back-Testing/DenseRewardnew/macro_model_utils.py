...
from test_model.env_dense_reward import TradingEnvironmentMacroV3,EvalTradingEnvironment
...
class MacroTraderModel:
    ...
    def test_benchmark(self, data, data_raw,  five_level_data, preferred_timeframe, model=None, market_cap="large", scenario="large",
        ...
        env = EvalTradingEnvironment(data, data_raw, eval=True,
                                        preferred_timeframe=390, initial_inventory=10000)
        use_stacked_env=True
        if use_stacked_env:
            vec_env_kwargs = {"data": data, "raw_data": data_raw, "eval": True, "preferred_timeframe": 390,
                            "initial_inventory": 10000}
            vec_env = make_vec_env(env_id = EvalTradingEnvironment, env_kwargs = vec_env_kwargs)
            stacked_env = VecFrameStack(vec_env, n_stack=5)
            env = stacked_env
