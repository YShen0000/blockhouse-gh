...
from test_model.data_handler_v2 import  fetch_data
from test_model.data_handler_test import DataProcessor
...
def get_data(data_dir, ticker, start_time, end_time):
    ...
    if os.path.exists(processed_filename):
        ...
        tech=DataProcessor()
        processed_data = tech.process_data(data, forecast_steps, n_jobs=4)
...
def run_equities_sell_inference(ticker, day_of_backtest, timeframe=390, inventory=10000, bt_data=None, trade_set_counter=1, market_cap_value=None):
    ...
    columns_to_drop = [
        'Unnamed: 0', 'log_returns_x', 'log_returns_y', 
        'log_stock_price_x', 'log_stock_price_y', 
        'trade_sign_x', 'trade_sign_y', 'vpin_series', 
        'timestamp', 'datetime', 'ask_size', 'bid_size'
    ]
