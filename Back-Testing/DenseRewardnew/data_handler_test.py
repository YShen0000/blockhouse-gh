...
from test_model.total_features import main_pipeline
...
class DataProcessor():
    ...
    def process_data(self, data, forecast_steps, window_size=100, n_jobs=-1):        
        ...
        merged_data = main_pipeline(data)
       ...
       'forecast_transaction_cost_transaction_cost'], axis=1, inplace=True)
        ...
        unique_columns_data_processed = data_processed.columns
        unique_columns_merged_data = merged_data.columns.difference(data_processed.columns)

        # Merge only unique columns
        all_data = pd.merge(data_processed[unique_columns_data_processed.tolist()],
                            merged_data[['datetime'] + unique_columns_merged_data.tolist()],
                            on=['datetime'], how='inner')
        # all_data = pd.merge(data_processed, merged_data, on= ['datetime'], how='inner')
        return all_data
