...
def compute_components(data, shares, ticker, min_backtest_data):
    ...
    Spread_cost = (actual_price - mid_price) * shares
    Slippage = (actual_price - min_ask_price) * shares 
...
def simulate_strategy(trades, data, preferred_timeframe, ticker, min_backtest_data=None):
    ...
    opportunity_cost_vs_close = (vwap_execution_price - close_price) * sum(shares_traded)
    opportunity_cost_vs_open = (vwap_execution_price-open_price) * sum(shares_traded)
