...
def get_full_day_expected_price(symbol, start_time, end_time):
    ...
    while current_time <= end_time:
        # Convert timestamp to nanoseconds
        end_timestamp = int(current_time.timestamp() * 1_000_000_000)
        start_timestamp = end_timestamp - (60 * 1_000_000_000)  # Subtract 1 minute
        ...
        try:
            ...
            for quote in quotes:
                max_bid_price = max(max_bid_price, quote.bid_price)
                min_ask_price = min(min_ask_price, quote.ask_price)
                bid_prices.append(quote.bid_price)
                bid_sizes.append(quote.bid_size * 100)
                ask_sizes.append(quote.ask_size * 100)
                ask_prices.append(quote.ask_price)
            ...
            if bid_prices and ask_prices:
                ...
                print(f"No data found for the given timestamp: {current_time}")

        except Exception as e:
            print(f"Error fetching data for {current_time}: {str(e)}")

        # Increment the time by 1 minute
        current_time += timedelta(minutes=1)

    # Create DataFrame from collected data
    minute_bid_ask_quotes = pd.DataFrame(minute_data)

    # Save DataFrame to a CSV file
    if not minute_bid_ask_quotes.empty:
        start_time_str = start_time.strftime('%Y-%m-%d_%H-%M-%S')
        minute_bid_ask_quotes.to_csv(f'BacktestData/minute_{symbol}_{start_time_str}.csv', index=False)
    else:
        print("No data collected for the given time range.")
