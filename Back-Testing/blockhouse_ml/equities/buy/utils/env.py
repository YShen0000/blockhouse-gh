...
#         # Initialize the client with your API key
#         client = RESTClient(self.api_key)
#         timestamp_est = timestamp_utc.tz_convert('America/New_York')

#         # Convert timestamp to nanoseconds
#         end_timestamp = int(timestamp_est.timestamp() * 1_000_000_000)
#         start_timestamp = end_timestamp - (60 * 1_000_000_000)  # Subtract 1 minute from the timestamp for start time
...
#         return expected_price, bid_sizes, ask_sizes, bid_prices, ask_prices
    ...
    def step(self, action):
        ...
        try:
            timestamp = pd.to_datetime(timestamp_str)
            timestamp_utc = timestamp.tz_localize('America/New_York').tz_convert('UTC')  # Assuming data is in EST
            timestamp_str_utc = timestamp_utc.strftime('%Y-%m-%d %H:%M:%S')
        except Exception as e:
            print(f"Error converting timestamp to UTC: {e}")
            timestamp_str_utc = timestamp_str  # Fallback to original if conversion fails

        # expected_price, bid_sizes, ask_sizes, bid_prices, ask_prices = self.get_expected_price(ticker_symbol, timestamp_str_utc)
        ...
        # next_timestamp = self.get_next_valid_market_timestamp(self.timestamp)
...
#         kappa = 0.1
#         actual_price = execution_price
#         available_liquidity = trade_row['volume']
#         expected_price_movement = expected_price - actual_price

#         # Base reward: price improvement relative to expected price
#         base_reward = (expected_price - actual_price) * volume_traded

#         # Liquidity-based slippage penalty
#         if available_liquidity > 0:
#             volume_ratio = min(volume_traded / available_liquidity, 1)
#         else:
#             volume_ratio = 1
        
#         slippage_penalty = abs(actual_price - expected_price) * volume_ratio * volume_traded

#         # Order type specific rewards
#         if order_type < 0.5:  # Market order
#             execution_speed_reward = 0.03 * volume_traded  # Increased reward for quick fills
#             reward = base_reward + execution_speed_reward - slippage_penalty
#         else:  # Limit order
#             patience_reward = 0.01 * volume_traded * sum(order['time_active'] for order in self.live_orders)
#             price_improvement_reward = max(0, (expected_price - actual_price) * volume_traded)
#             reward = base_reward + patience_reward + price_improvement_reward - slippage_penalty

#         # Opportunity cost
#         opp_cost = self.current_average_trade_price - self.start_trade_price #* volume_traded

#         # Inventory penalty to encourage completing the order
#         inventory_penalty = 0.02 * self.remaining_shares_to_buy

#         # Time decay penalty
#         time_decay = 0.005 * self.current_step * volume_traded

#         # Volatility adjustment
#         volatility = trade_row.get('ATR_1', 0)  # Assuming ATR_1 is available in trade_row
#         volatility_adjustment = 0.1 * volatility * volume_traded
...
#         # Apply utility theory for risk-aversion
#         risk_adjusted_reward = total_reward - (kappa * (total_reward ** 2))

#         print(f"Execution Price: {actual_price}, Expected Price: {expected_price}")
#         print(f"Base Reward: {base_reward}, Slippage Penalty: {slippage_penalty}")
#         print(f"Opportunity Cost: {opp_cost}, Inventory Penalty: {inventory_penalty}")
#         print(f"Time Decay: {time_decay}, Volatility Adjustment: {volatility_adjustment}")
#         print(f"Total Reward: {total_reward}, Risk-Adjusted Reward: {risk_adjusted_reward}")
