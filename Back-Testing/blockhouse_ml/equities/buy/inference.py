...
class EquitiesBuyInference:
    ...
    def run_pipeline(self, ticker, start_timestamp = (datetime.today() - timedelta(days=7)).strftime('%Y-%m-%d'), end_timestamp=datetime.today().strftime('%Y-%m-%d'), timeframe=390, inventory=10000, trade_set_counter=1):
        """
        ...
        micro_env.render()
        trade_details = pd.DataFrame(trade_details) # Must convert to data frame 
        trade_details = trade_details.rename(columns={'volume': 'shares'}) # Change name to shares, name used with twap and vwap
