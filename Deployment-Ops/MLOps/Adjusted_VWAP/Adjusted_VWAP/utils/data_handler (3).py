...
class InferenceDataHandler():
    ...
    def create_data_w_forecasts(self, data, steps = 390):
        ...
        def generate_trading_hours_timestamps(start_timestamp, forecast_steps):
            ...
            # Trading hours in UTC: 13:30 to 20:00 (9:30 AM - 4:00 PM EST)
            trading_start_time = pd.Timedelta(hours=13, minutes=30)
            trading_end_time = pd.Timedelta(hours=20)

            timestamps = []
            current_time = start_timestamp

            while len(timestamps) < forecast_steps:
                # Set trading start and end for the current day
                trading_start_utc = current_time.normalize() + trading_start_time
                trading_end_utc = current_time.normalize() + trading_end_time
                
                # Check if the current day is a holiday or weekend
                if trading_start_utc.date() in holidays.date or trading_start_utc.weekday() >= 5:
                    # Move to the next business day
                    current_time = (current_time + BDay(1)).normalize()
                    continue
                
                # Generate timestamps within trading hours
                current_time = max(trading_start_utc, current_time)  # Start at trading start time if before it
                while current_time <= trading_end_utc and len(timestamps) < forecast_steps:
                    timestamps.append(current_time)
                    current_time += pd.Timedelta(minutes=1)
                
                # Move to the next business day if out of trading hours
                current_time = (current_time + BDay(1)).normalize()
