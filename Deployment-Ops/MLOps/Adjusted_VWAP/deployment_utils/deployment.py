...
def create_sagemaker_endpoint(model_name,  endpoint_config_name, endpoint_name, role_arn, instance_type="ml.g4dn.xlarge"):
    ...
    print(f" Endpoint name: {endpoint_name}, model name: {model_name} created successfully.")
    try:
        inference_component = sagemaker_client.create_inference_component(
        ...
        if e.response['Error']['Code'] == 'ValidationException':
            if "Cannot create already existing inference component" in str(e):
                print(f"Inference component '{model_name}-inference-component' already exists.")
...
if __name__ == "__main__":
    ...
    inference_script = ""
    output_path = "model_adjusted_VWAP_BL_report.tar.gz"
    ...
    s3_key = f'production/version_{version}/model_adjusted_VWAP_BL_report.tar.gz'
