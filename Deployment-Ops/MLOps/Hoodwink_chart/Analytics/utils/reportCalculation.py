...
def main(csv_path, platform_type,file_id):
    ...
    trade_blotter = cache.get(key=cache_key)
    
    if trade_blotter is None:
        print('In Here')
        ...
        # 2. Generate Graph
        # generate_buffer_slippage = generate_graph(trade_blotter)
        # cache.set(key=f"{platform_type}_{file_id}_generate_buffer_slippage",value=generate_buffer_slippage,timeout=60 * 15)

        # Line Chart
        #stock = trade_blotter_filtered['Instrument'].unique().tolist()[0]
        #generate_buffer_line_chart = data_fetching_and_preprocessing.plot_line_chart(trade_blotter_filtered, stock, show_open=True, show_close=True, show_twap=True, show_vwap=True, show_hwoe=True, start_date=start_date, end_date=end_date)

        # 3. Trades with Most Slippage
        trades_most_slippage = calculate_trades_slippage(trade_blotter)
        cache.set(key=f"{platform_type}_{file_id}_trades_most_slippage",value=trades_most_slippage,timeout=60 * 15)
        
        # 4. Calculate Potential Savings
        calculate_saving = calculate_potential_savings(trade_blotter)
        cache.set(key=f"{platform_type}_{file_id}_calculate_saving",value=calculate_saving,timeout=60 * 15)
        ...
        start_date = trade_blotter_filtered['Activity Date'].min()
        end_date = trade_blotter_filtered['Activity Date'].max()
        generate_buffer_line_chart = data_fetching_and_preprocessing.plot_line_chart(trade_blotter_filtered, stock, show_open=True, show_close=True, show_twap=True, show_vwap=True, show_hwoe=True, start_date=start_date, end_date=end_date,hwoe_adjustment_factor=calculate_saving['scaling_factor'])
        cache.set(key=f"{platform_type}_{file_id}_generate_buffer_line_chart",value=generate_buffer_line_chart,timeout=60 * 15)
    ...
    # print("actionable_recommendation: ", actionable_recommendation)    
    elif cache.get(key=f"{platform_type}_{file_id}_trading_overview") is None:
        print("In elif cache")
        print(f"{platform_type}_{file_id}")
        ...
        generate_buffer_slippage = cache.get(key=f"{platform_type}_{file_id}_generate_buffer_slippage")
        generate_buffer_line_chart = cache.get(key=f"{platform_type}_{file_id}_generate_buffer_line_chart")
        trades_most_slippage = cache.get(key=f"{platform_type}_{file_id}_trades_most_slippage")
        calculate_saving = cache.get(key=f"{platform_type}_{file_id}_calculate_saving")
...
# csv_path = 'charles_schwab_trade_data_07_08_2024_19_09_45.csv'
