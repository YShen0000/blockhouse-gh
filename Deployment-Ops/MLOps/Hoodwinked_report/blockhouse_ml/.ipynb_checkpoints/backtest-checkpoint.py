...
from blockhouse_ml.plot_backtests import plot_metrics
from blockhouse_ml.equities.utils.fetch_merge_data import PolygonClient
from blockhouse_ml.equities.sell.utils.data_handler import DataProcessor
...
from blockhouse_ml.equities.sell.utils.benchmark_utils import get_full_day_expected_price
from blockhouse_ml.equities.sell.utils.benchmark_utils import (
...
from blockhouse_ml.equities.buy.utils.benchmark_utils import (
...
from blockhouse_ml.options.utils.benchmark_utils import (
...
def get_backtest_day_data(day_of_backtest, ticker):
    ...
    matches = data.index[data['datetime'] == target_datetime]

    if len(matches) == 0:
        raise ValueError(f"No data found for the target datetime: {target_datetime}")
    elif len(matches) > 1:
        raise ValueError(f"Multiple entries found for the target datetime: {target_datetime}")
    else:
        startIdx = matches.item()
...
def load_backtest_data(ticker, day_of_backtest):
    ...
    if os.path.exists(filename):
        ...
        day_of_backtest_data = save_backtest_data(ticker, day_of_backtest)  
...
def run_backtest_day(ticker='AAPL', inventory=1000, day_of_backtest=(datetime.now(pytz.UTC) - timedelta(days=2)).strftime('%Y-%m-%d'),
                 is_equity=True, is_sell=True, plotter=False):
...
def run_backtest_list(user_trades_data):
    model_trade_info = []
    ...
    for i, row in user_trades_data.iterrows():
        ...
        model_trades = run_backtest_day(ticker, inventory, day_of_backtest, is_equity, is_sell)
    ...
    columns = ['date', 'ticker', 'inventory', 'is_sell', 'is_equity', 'price']
    model_trade_info = pd.DataFrame(model_trade_info, columns=columns)
    
    return model_trade_info

    
# def run_backtest_list(csv_file_path):
