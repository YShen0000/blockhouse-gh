...
def generate_trading_overview(trades):
    ...
    # print("----------------------------------------------------------------")
    # print(f"Timeframe Analyzed: {timeframe_analyzed}")
    # print(f"Assets analyzed: {assets_analyzed}")
    # print(f"Frequency of Trade: {total_trades} -- {trades_per_month} trades/month -- {trades_per_week} trades/week")
...
def generate_graph(enriched_trades,scaling_factor=0.25):
    # slippage_summary = {
    #     'Open': enriched_trades['Slippage_PCT_Open'].mean(),
    #     'Close': enriched_trades['Slippage_PCT_Close'].mean(),
    #     'TWAP': enriched_trades['Slippage_PCT_TWAP'].mean(),
    #     'VWAP': enriched_trades['Slippage_PCT_VWAP'].mean(),
    #     'HWOE': enriched_trades['Slippage_PCT_HWOE'].mean()
    # }

    # slippage_df = pd.DataFrame(list(slippage_summary.items()), columns=['Strategy', 'Slippage'])
    
    # plt.figure(figsize=(10, 6))
    # plt.bar(slippage_df['Strategy'], slippage_df['Slippage'], color=['blue', 'orange', 'green', 'red', 'purple'])
    # plt.xlabel('Strategy')
...
def calculate_trades_slippage(enriched_trades):
    ...
    # print('Top Trades with Slippage:')
    # print("----------------------------------------------------------------")
    # print(top_slippage_trades[['Activity Date', 'Trans Code' ,'Instrument', 'Price', 'TWAP_Price', 'Slippage_PCT_TWAP', 'Quantity', 'Market_Condition']])
    # print("----------------------------------------------------------------")
    ...
    # return {
    #     'top_slippage_trades': top_slippage_trades[['Activity Date', 'Trans Code' ,'Instrument', 'Price', 'TWAP_Price', 'Slippage_PCT_TWAP', 'Quantity', 'Market_Condition']]
    # }
...
def calculate_potential_savings(enriched_trades):
    ...
    # print("Total BUYs: ", buy_quantity.sum())
    # print("Total SELLs: ", sell_quantity.sum())
    ...
    # print(results_df)
    # print("----------------------------------------------------------------")
    # print(f"The total savings from executing the optimal strategy is: ${adjusted_hwoe_savings:.2f}")
...
#         print(f"Debuggg: {buys}")
        ...
        # Print all the results for the current week
#         print(f"Week {week_summary['week']}")
#         print(f"Min Buy Price: {week_summary['min_buy_price']}")
#         print(f"Max Sell Price: {week_summary['max_sell_price']}")
#         print(f"Total Buy Quantity: {week_summary['total_buy_quantity']}")
#         print(f"Total Sell Quantity: {week_summary['total_sell_quantity']}")
#         print(f"Actual Buy Cost: {week_summary['actual_buy_cost']}")
#         print(f"Actual Sell Revenue: {week_summary['actual_sell_revenue']}")
#         print(f"Optimal Buy Cost: {week_summary['optimal_buy_cost']}")
#         print(f"Optimal Sell Revenue: {week_summary['optimal_sell_revenue']}")
#         print(f"Savings from optimal buy strategy: {week_summary['savings_buy']}")
#         print(f"Savings from optimal sell strategy: {week_summary['savings_sell']}")
#         print(f"Total Savings: {week_summary['total_savings']}")
#         print(f"Slippage in Buy: {week_summary['slippage_buy']}")
#         print(f"Slippage in Sell: {week_summary['slippage_sell']}")
#         print(f"Most Active Weekday: {day_dict[week_summary['most_active_weekday']]}")
#         print(f"Most Active Hour: {week_summary['most_active_hour']}")
...
def optimize_limits(trades, w1, w2):
    ...
    if result.success:
        ...
        # print(f"Optimized Buy Limit Percentage: {optimized_percentages[0]:.2f}%")
        # print(f"Optimized Sell Limit Percentage: {optimized_percentages[1]:.2f}%")
...
def determine_optimal_limits(trades, w1, w2):
    ...
    # print(f"Total Buy Savings: ${total_buy_savings:.2f}")
    # print(f"Total Sell Savings: ${total_sell_savings:.2f}")
...
def grid_search(trades, w1_values, w2_values):
    ...
    for w1 in w1_values:
        for w2 in w2_values:
            #print(f"Testing with w1: {w1}, w2: {w2}")
            try:
...
# Example usage:
# csv_path = 'trade_data_07_08_2024_18_51_46.csv'

# trade_blotter = pd.read_csv(csv_path)
# trade_blotter = preprocess_data(trade_blotter)

# # Ensure your DataFrame has the necessary columns and 5d_MA calculated
# # Assuming trade_blotter['5d_MA'] is already calculated as per your previous steps

# w1_values = np.linspace(0.1, 5, 10)  # Range of w1 values to test
# w2_values = np.linspace(0.1, 5, 10)  # Range of w2 values to test

# best_result, best_w1, best_w2 = grid_search(trade_blotter, w1_values, w2_values)

# print(f"Best w1: {best_w1}, Best w2: {best_w2}")
# print(best_result)
...
def calculate_limits(enriched_trades):
    ...
    for ticker, limits in optimal_limits_data.items():
        # final_report.append(f"- **{ticker}** – Set BUY limit for {limits[0]:.1f}% below 5 day MA, SELL limit {limits[1]:.1f}% above")
        final_report.append({
            "ticker": ticker,
...
def actionable_recommendations(trade_blotter):
    ...
    # print(f"The best weekday to trade is: {trade_blotter['HWOE_Day'].mode()[0]}")
    # print(f"The best hour to trade is: {trade_blotter['HWOE_Time'].mode()[0]}")

    # Calculate total savings over the period
    total_savings = weekly_analysis['total_savings'].sum()
    # print(f"The total savings from executing the optimal strategy is: ${total_savings:.2f}")
...
#     # Line Chart
#     stock = trade_blotter['Instrument'].unique().tolist()[0]
#     start_date = trade_blotter['Activity Date'].min()
#     end_date = trade_blotter['Activity Date'].max()
#     generate_buffer_line_chart = data_fetching_and_preprocessing.plot_line_chart(
#         trade_blotter, stock, show_open=True, show_close=True, show_twap=True, 
#         show_vwap=True, show_hwoe=True, start_date=start_date, end_date=end_date, 
#         hwoe_adjustment_factor=calculate_saving['scaling_factor'])

#     # Trades with Most Slippage
...
from backtest_lib.backtest import run_blockhouse_list  # Ensure correct import path

# from your_backtesting_module import run_blockhouse_list  # Ensure correct import path

def main(csv_path, platform_type):
    # Read csv
    trade_blotter = pd.read_csv(csv_path)
    trade_blotter = trade_blotter[:2]
    # Preprocess by platform
    trade_blotter = preprocess_by_platform(trade_blotter, platform_type)
    ...
    # **Run Blockhouse Backtest and Replace HWOE**
    blockhouse_backtest_results = run_blockhouse_list(csv_path)

    # Directly use slippage values instead of calculating HWOE
    trade_blotter['Model_Slippage'] = blockhouse_backtest_results['model_slippage']
    trade_blotter['TWAP_Slippage'] = blockhouse_backtest_results['twap_slippage']
    trade_blotter['VWAP_Slippage'] = blockhouse_backtest_results['vwap_slippage']
    
    # Include any other backtest results you need
    trade_blotter['Model_Market_Impact'] = blockhouse_backtest_results['model_market_impact']
    trade_blotter['Model_Transaction_Cost'] = blockhouse_backtest_results['model_transaction_cost']

    # Now, there is no need to calculate HWOE or price-based slippage manually.

    # Generate overview
    trading_overview = generate_trading_overview(trade_blotter)
    
    # Generate slippage graph
    calculate_saving = calculate_potential_savings(trade_blotter)  # Now based on direct slippage data
    generate_buffer_slippage = generate_graph(trade_blotter, calculate_saving['scaling_factor'])
    ...
    generate_buffer_line_chart = data_fetching_and_preprocessing.plot_line_chart(
        trade_blotter, stock, show_open=True, show_close=True, show_twap=True, 
        show_vwap=True, start_date=start_date, end_date=end_date
    )
    ...
    return {
        ...
        'calculate_potential_savings': calculate_saving,
        'actionable_recommendations': actionable_recommendation,
        'blockhouse_backtest_results': blockhouse_backtest_results  # Include backtest results in output
    }
...
#     trade_blotter = preprocess_by_platform(trade_blotter, platform_type)
#     print(trade_blotter)
#     trade_blotter['is_sell'] = trade_blotter['Trans Code'].str.lower() == 'sell'
#     trade_blotter['is_equity'] = True
...
#     # **Run Blockhouse Backtest and Replace HWOE**
#     blockhouse_backtest_results = run_blockhouse_list(csv_path)
#     # print(blockhouse_backtest_results)
#     # Replace HWOE-related fields with blockhouse backtest results
#     print(blockhouse_backtest_results.columns)
#     trade_blotter['HWOE_Price'] = blockhouse_backtest_results['price']
#     trade_blotter['HWOE_Day'] = pd.to_datetime(blockhouse_backtest_results['date']).dt.day_name()
#     trade_blotter['HWOE_Time'] = pd.to_datetime(blockhouse_backtest_results['date']).dt.time
...
csv_path = 'D:\Deployment_code\Blockhouse-ML\Hoodwinked_report\charles_schwab_trade_data_07_22_2024_20_46_51.csv'
