...
def preprocess_data_merged(trade_blotter):
    ...
    for ticker in tickers:
        # if is_fund(ticker):
        #     print(f"Skipping {ticker} as it is a fund.")
        #     continue
        try:
            data = yf.Ticker(ticker).history(period='2y', interval='1h')
            if not data.empty:
...
#             if trans_code.lower() == 'buy':
#                 HWOE_row = week_data.loc[week_data['Low'].idxmin()]
#                 HWOE_price = HWOE_row['Low']
#             elif trans_code.lower() == 'sell':
#                 HWOE_row = week_data.loc[week_data['High'].idxmax()]
#                 HWOE_price = HWOE_row['High']
#             else:
#                 HWOE_price = None
#                 HWOE_row = None
            ...
            if HWOE_row is not None:
                HWOE_days.append(HWOE_row.name.strftime('%A'))  # Day of the week
                HWOE_times.append(HWOE_row.name.strftime('%H:%M:%S'))  # Time of the trade
            else:
...
def slippage_pie_chart(trade_blotter_filtered, stock, start_date, end_date, benchmark):
    ...
    if trade_df.empty:
        return JsonResponse({"message": f"No trades found for {stock} between {start_date} and {end_date}."}, status=404)
    ...
    # Adding hover data
    trade_df['Hover_Text'] = trade_df['Trans Code'] + " " + trade_df['Quantity'].astype(str) + " @ $" + trade_df['Price'].astype(str)
    hover_texts = trade_df.groupby('Slippage_Class')['Hover_Text'].apply(lambda x: ' , '.join(x))
...
def calculate_excess_returns(data, stock, benchmarks, start_date, end_date, hwoe_adjustment_factor=0.25):
    ...
    filtered_data = data[(data['Instrument'] == stock) & 
                         ...
                         (data['Activity Date'] <= end_date)].copy()
...
def calculate_aggregated_excess_returns(data, benchmarks, start_date, end_date, hwoe_adjustment_factor=0.25):
    ...
    filtered_data = data[(data['Activity Date'] >= start_date) & 
                         (data['Activity Date'] <= end_date)].copy()
...
def get_excess_returns_data(trade_blotter_filtered, stock, show_open, show_close, show_twap, show_vwap, show_hwoe, start_date, end_date, hwoe_adjustment_factor=0.3, aggregate=False):
    start_date = pd.to_datetime(start_date)
    end_date = pd.to_datetime(end_date)
    ...
    for benchmark, returns in excess_returns.items():
        ...
        if window_length < 2:
            print(f"Not enough data points for smoothing benchmark {benchmark}. Plotting raw data.")
            returns[f'{benchmark}_Excess_Returns'] = returns[f'{benchmark}_Excess_Returns']
        else:
            returns[f'{benchmark}_Excess_Returns'] = savgol_filter(returns[f'{benchmark}_Excess_Returns'], window_length=window_length, polyorder=2)
        
        processed_data[benchmark] = returns[['Activity Date', f'{benchmark}_Excess_Returns']].to_dict(orient='records')
    
    return processed_data

def plot_excess_returns(trade_blotter_filtered, stock, show_open, show_close, show_twap, show_vwap, show_hwoe, start_date, end_date, hwoe_adjustment_factor=0.25, aggregate=True):
    processed_data = get_excess_returns_data(trade_blotter_filtered, stock, show_open, show_close, show_twap, show_vwap, show_hwoe, start_date, end_date, hwoe_adjustment_factor, aggregate)
    # return excess_returns
    #print(processed_data)
    return processed_data
    # plt.figure(figsize=(14, 7))
    # for benchmark, returns in excess_returns.items():
    #     plt.plot(returns['Activity Date'], returns[f'{benchmark}_Excess_Returns'], label=benchmark)
    
    # plt.xlabel('Date')
    # plt.ylabel('Excess Returns (bps)')
    # plt.title(f'Excess Returns Over Time By Strategy for {stock}')
    # plt.legend()
    # plt.grid(True)
    # plt.show()
...
def calculate_excess_returns_original(data, stock, benchmarks):
    ...
    for benchmark in benchmarks:
        ...
        filtered_data[f'{benchmark}_Excess_Returns'] = -filtered_data[f'Slippage_{benchmark}'].cumsum()
        # excess_returns[benchmark] = filtered_data[['Activity Date', f'{benchmark}_Excess_Returns']]
        excess_returns[benchmark] = filtered_data[['Activity Date', f'{benchmark}_Excess_Returns']]
    
    return excess_returns

def plot_line_chart(trade_blotter_filtered, stock, show_open, show_close, show_twap, show_vwap, show_hwoe, start_date, end_date, hwoe_adjustment_factor=0.25, aggregate=True):
    start_date = pd.to_datetime(start_date)
    end_date = pd.to_datetime(end_date)
    ...
    for benchmark, returns in excess_returns.items():
        ...
        returns[f'{benchmark}_Excess_Returns_Smooth'] = returns[f'{benchmark}_Excess_Returns']
        processed_data[benchmark] = returns[['Activity Date', f'{benchmark}_Excess_Returns_Smooth']]
    ...
    # excess_returns = calculate_excess_returns_original(trade_blotter_filtered, stock, benchmarks)
    ...
    for benchmark, data in processed_data.items():
        plt.plot(data['Activity Date'], data[f'{benchmark}_Excess_Returns_Smooth'], label=benchmark)
    
    plt.xlabel('Date')
    plt.ylabel('Excess Cost Savings ($)')
    plt.title(f'Excess Returns Over Time By Strategy for {"All Stocks" if aggregate else stock}')
    plt.legend()
    plt.grid(True)
...
def calculate_slippage(trade_data, data_dict, selected_trade='All Trades', hwoe_adjustment_factor=0.25):
    days_after_trade = ['1 Day', '2 Days', '3 Days', '4 Days']
    price_types = ['Open', 'Close', 'VWAP', 'TWAP', 'HWOE']
    slippage_matrix = pd.DataFrame(index=price_types, columns=days_after_trade, dtype=float).fillna(0)
    ...
    for _, trade in trades.iterrows():
        ...
        while days_counted < 4:
            ...
            if date_after_trade.weekday() >= 5:
                continue
            market_subset = market_data.loc[market_data.index.date == date_after_trade.date()]
            ...
            if not market_subset.empty:
                ...
                vwap_price = np.sum(market_subset['Close'] * market_subset['Volume']) / np.sum(market_subset['Volume'])
                ...
                if trans_code.lower() == 'buy':
                    slippage_values = [
                        ...
                        quantity * (trade_price - hwoe_price) * adjustment_factor  # Apply adjustment factor only for "All Trades"
                    ]
                else:
                    slippage_values = [
                        ...
                        quantity * (hwoe_price - trade_price) * adjustment_factor  # Apply adjustment factor only for "All Trades"
                    ]
                
                for price_type, slippage_value in zip(price_types, slippage_values):
                    slippage_matrix.at[price_type, days_after_trade[days_counted]] += slippage_value
            days_counted += 1

    return slippage_matrix.to_dict()

# def calculate_blockhouse_pe(data, date, trans_code, data_dict, ticker):
#     start_of_week, end_of_week = get_week_range(date)
#     week_data = data_dict[ticker].loc[start_of_week:end_of_week]
...
def identify_optimal_points(filtered_data, trade_datetime, trans_code):
    # start_of_week = trade_date - timedelta(days=trade_date.weekday())
    # end_of_week = start_of_week + timedelta(days=4)

    # filtered_data = filtered_data.set_index('Datetime')
    
    # before_trade = filtered_data[(filtered_data.index < trade_date) & (filtered_data.index >= start_of_week)]
    # after_trade = filtered_data[(filtered_data.index > trade_date) & (filtered_data.index <= end_of_week)]
    # Ensure the index is a DatetimeIndex
    if not isinstance(filtered_data.index, pd.DatetimeIndex):
        filtered_data = filtered_data.set_index('Datetime')
...
def plot_slippage_analysis(filtered_data, trade_datetime, trade_price, optimal_before, optimal_after, optimal_before_date, optimal_after_date, slippage_before, slippage_after, trans_code):
    ...
    results = { 
        "plot_data": plot_data.to_dict(orient='index'),  # Convert DataFrame to dictionary for JSON 
...
# def plot_slippage_analysis(filtered_data, trade_date, trade_price, optimal_before, optimal_after, optimal_before_date, optimal_after_date, slippage_before, slippage_after, trans_code):
#     start_plot_date = trade_date - timedelta(days=3)
#     end_plot_date = trade_date + timedelta(days=3)

#     # Convert dictionary back to DataFrame for plotting
#     plot_data = pd.DataFrame.from_dict(filtered_data, orient='index')
#     plot_data.index = pd.to_datetime(plot_data.index)

#     # Filter the data for plotting range
#     plot_data = plot_data[(plot_data.index >= start_plot_date) & (plot_data.index <= end_plot_date)]
...
def candle_stick_trade_option(trade_blotter_filtered):
    trade_options = [f"{idx}: {row['Instrument']} {row['Trans Code']} {row['Quantity']} @ ${row['Price']} on {row['Activity Date']}" for idx, row in trade_blotter_filtered.iterrows()]
    return trade_options
...
def setup_slippage_analysis(trade_blotter_filtered, data_dict, selected_trade, selected_benchmarks='All'):
    ...
    optimal_before, optimal_after, optimal_before_date, optimal_after_date = identify_optimal_points(market_data, trade_datetime, trans_code)
    # optimal_before, optimal_after, optimal_before_date, optimal_after_date = identify_optimal_points(market_data, trade_date, trans_code)
    ...
    result_data = plot_slippage_analysis(market_data, trade_datetime, trade_price, optimal_before, optimal_after, optimal_before_date, optimal_after_date, slippage_before, slippage_after, trans_code)

    if 'All' in selected_benchmarks:
        selected_benchmarks = benchmark_options
    benchmark_prices = preprocess_benchmark_prices(trade_details, result_data['plot_data'], selected_benchmarks) 
    ...
    # trade_dropdown.observe(on_trade_change, names='value')
    # on_trade_change({'new': trade_dropdown.value})

# setup_slippage_analysis(trade_blotter_filtered, data_dict)
...
def collect_slippage_data(trade_details, benchmark_prices):
    ...
    for benchmark, price in benchmark_prices.items():
        slippage = price - trade_price if trans_code.lower() == 'buy' else trade_price - price
...
def calculate_slippage_sums_plaid(trade_data, selected_trade='All Trades', hwoe_adjustment_factor=0.25):
    slippage_types = ['Slippage_Open', 'Slippage_Close', 'Slippage_TWAP', 'Slippage_VWAP', 'Slippage_HWOE']
    strategies = ['Open', 'Close', 'TWAP', 'VWAP', 'HWOE']
    ...
    for strategy, slippage_type in zip(strategies, slippage_types):
        if strategy == 'HWOE':
            slippage_sums[strategy] = (trades[slippage_type] * trades['Quantity'] * hwoe_adjustment_factor).sum()
        else:
            slippage_sums[strategy] = (trades[slippage_type] * trades['Quantity']).sum()
