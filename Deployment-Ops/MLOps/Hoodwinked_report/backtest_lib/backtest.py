...
from backtest_lib.plot_backtests import plot_metrics
from backtest_lib.equities.utils.fetch_merge_data import PolygonClient
from backtest_lib.equities.utils.data_handler import DataProcessor
...
def run_blockhouse_list(csv_file_path):
    ...
    list_of_backtests = list_of_backtests[:1]
    list_of_backtests['is_sell'] = list_of_backtests['Action'].str.lower() == 'sell'
    list_of_backtests['is_equity'] = True
    list_of_backtests['Date'] = pd.to_datetime(list_of_backtests['Date'], format='%m/%d/%Y').dt.strftime('%Y-%m-%d')
    comparison_info = []
    # Loop through each row of the DataFrame (each trade the user made)
    for i, row in list_of_backtests.iterrows():
    
        print(list_of_backtests)
        day_of_backtest, ticker, inventory, is_sell, is_equity = row['Date'], row['Symbol'], row['Quantity'], row['is_sell'], row['is_equity']
