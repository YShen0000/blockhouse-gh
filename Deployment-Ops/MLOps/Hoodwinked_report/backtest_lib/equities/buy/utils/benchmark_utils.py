...
def calculate_vwap(ask_prices, ask_sizes, size_of_slice):
    # VWAP calculation: sum(price * size) / sum(size)
    cum_sum_size = 0
    for idx, size in enumerate(ask_sizes):
        # print(idx)
        cum_sum_size += size
    ...
    ask_prices = np.array(ask_prices[:idx+1])
    ask_sizes = np.array(ask_sizes[:idx+1])

    # Ensure bid_sizes is not zero to avoid division by zero
    if np.sum(ask_sizes) == 0:
        print("ask_size zero")
        return 0.0  # Handle edge case

    vwap = np.sum(ask_prices * ask_sizes) / np.sum(ask_sizes)
...
def get_actual_price(data, ticker, size_of_slice, min_backtest_data):
    ...
    ask_sizes = row["ask_sizes"].iloc[0]
    min_ask_price = row["min_ask_price"].iloc[0]

    actual_price = calculate_vwap(ask_prices, ask_sizes, size_of_slice)
...
def compute_components(data, alpha, lambda_, shares, ticker, min_backtest_data):
    ...
    Spread_cost = (avg_ask_price - avg_bid_price) * shares # Should be the same based on what i read but is negative
    Slippage = (actual_price - min_ask_price) * shares # Should be negative, flipped
...
def get_twap_trades(data, target_inventory, preferred_timeframe):
    ...
    for step in range(min(total_steps, preferred_timeframe)):
        size_of_slice = min(int(np.ceil(twap_shares_per_step)), target_inventory - accumulated_inventory)
...
def get_vwap_trades(data, target_inventory, preferred_timeframe, if_limit=False):
    ...
    for step in range(min(total_steps, preferred_timeframe)):
        ...
        size_of_slice = min(int(np.ceil(size_of_slice)), target_inventory - accumulated_inventory)
        accumulated_inventory += int(np.ceil(size_of_slice))
        if if_limit:
...
def simulate_strategy(trades, data, preferred_timeframe, ticker, min_backtest_data=None):
    """
    ...
    for idx in range(len(trades)):
        ...
        reward = compute_components(corresponding_data, alpha, lambda_, shares, ticker, min_backtest_data)
