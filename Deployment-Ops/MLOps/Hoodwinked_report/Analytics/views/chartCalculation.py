...
def chartCalculation(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # plt.plot(range(len(case_chunk['almgren_100000'])), case_chunk['almgren_100000'], label="Realized Transaction Costs")
            # plt.plot(range(len(case_chunk['almgren_100000'])), case_chunk['aapl_lstm_100000'], label="Predicted Transaction Costs")
            # for i in range(10):
            #     plt.axvline(x=locations[i], color='red', linestyle='--')
            # plt.xlabel('Time(Trading week from 6/20-6/27 broken by 5 minute increments)')
            # plt.ylabel('Almgren-Chriss per Share Transaction Costs')
            # plt.title('Using Optimal Trading Times to reduce Transaction Costs')
            # plt.legend()
            ...
            # plt.plot(np.log10(x_axis), lstm_vs_twap)

            # plt.plot(np.log10(x_axis), lstm_vs_vwap)
            # Getting the graph values
            # Format the data into a list of dictionaries suitable for Recharts
            ...
            # user_order_size = 10000
            sp_returns = 0.227
            # def calculate_cost_savings(user_benchmark, user_time_period, user_order_size, sp_returns):
            #     calc_func = lstm_twap_cost_savings_calc if user_benchmark == "TWAP" else lstm_vwap_cost_savings_calc
            #     time_multiplier = 52 if user_time_period == "week" else 52 * 7 if user_time_period == "day" else 1
            #     cost_savings_dollars = calc_func(user_order_size) * time_multiplier
            #     cost_savings_percent = cost_savings_dollars / (user_order_size * 200)
            #     excess_returns = cost_savings_percent / sp_returns
            #     return cost_savings_dollars, cost_savings_percent, excess_returns
            # if user_asset_class == "Equities":
            #     result = calculate_cost_savings(user_benchmark, user_time_period, user_order_size, sp_returns)
            #     print(result)
            ...
            #     calc_func = lstm_twap_cost_savings_calc if user_benchmark == "TWAP" else lstm_vwap_cost_savings_calc
            #     time_multiplier = 52 if user_time_period == "week" else (52 * 5) if user_time_period == "day" else 12 if user_time_period == "month" else 1
            #     cost_savings_dollars = calc_func(user_order_size) * time_multiplier * user_frequency
            #     cost_savings_percent = (cost_savings_dollars / (user_order_size * 200)) * 100
            #     cost_savings_avg_bps = cost_savings_percent * 100
            #     excess_returns = (cost_savings_percent / (sp_returns * 100)) * 100
            #     return cost_savings_dollars, cost_savings_avg_bps, excess_returns
            # if user_asset_class == "Equities":
