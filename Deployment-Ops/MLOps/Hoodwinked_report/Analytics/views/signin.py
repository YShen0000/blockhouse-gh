...
def generate_referral_code(user_id: str, length: int = 10):
    # Use a secure random identifier combined with user_id
    ...
    # Encode this integer to a Base62 string
...
def get_referral_link(request):
    ...
    if auth_header:
        ...
        try:
            ...
            email = decoded.get('email')  # Get the email from the payload
            
            # Query the database to get user_id corresponding to the email
            # user = User.objects.get(email=email)
            # user_id = user.id
