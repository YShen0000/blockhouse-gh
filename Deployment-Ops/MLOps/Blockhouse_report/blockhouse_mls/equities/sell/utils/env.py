...
class CustomTradingEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        # Adjust the size of the slice by scaling factor and step count
        if self.step_counter == 1:
            # For the first step, trade half of the initial remaining inventory
            size_of_slice = action[0] * self.remaining_inventory * 0.5

        elif self.step_counter > 4:
            # For steps beyond the fourth, use a fraction of the previous slice to progressively reduce the amount
            size_of_slice = action[0] * self.prev_slice
            self.prev_slice = 0.9 * self.prev_slice  # Reduce the previous slice by 10% for the next step

        else:
            # For steps between 2 and 4, calculate based on scaling factor and remaining inventory
            size_of_slice = action[0] * self.remaining_inventory * scaling_factor
            size_of_slice = int(np.ceil(size_of_slice))
            self.prev_slice = self.remaining_inventory  # Update prev_slice for future adjustments

        # Ensure the slice size is at least 1 if there's any remaining inventory
        size_of_slice = int(np.ceil(size_of_slice))

        # Adjust timing of the slice to stay within the remaining timeframe
        timing_of_slice = int(np.ceil(action[1]))
        ...
        # If time is running out, use up all remaining inventory
        if self.elapsed_time >= self.preferred_timeframe:
            size_of_slice = self.remaining_inventory

        # Take action and update inventory
...
class TradingEnvironmentMicro(gym.Env):
    ...
    def _add_noise_to_action(self, action):
        ...
        noise_action_0 = np.random.normal(0, 0.3)  # Small noise for order type
        action[0] += noise_action_0
        action[0] = np.clip(action[0], self.action_space.low[0], self.action_space.high[0])
    ...
    def step(self, action):
        ...
        mid_pricings = self.get_best_mid_price(self.data, action_type="sell")
