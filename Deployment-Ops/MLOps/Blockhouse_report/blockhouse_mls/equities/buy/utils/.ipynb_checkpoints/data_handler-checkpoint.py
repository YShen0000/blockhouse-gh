...
from blockhouse_mls.equities.utils.fetch_merge_data import fetch_and_merge_data
...
class DataProcessor():
    ...
    def add_technical_indicators(self, data):
        ...
        # Add all indicators
        print("data before indicator", data)
        data_with_indicators = indicators.add_all_indicators()
        print("data before droppping na", data_with_indicators)
        # Discard NaN values 
        data_filtered = data_with_indicators.dropna()
        print("data after droppping na", data_with_indicators)

        return data_filtered

    def process_data(self, data, forecast_steps, window_size=100, n_jobs=-1):        
        """
...
class InferenceDataHandler():
    ...
    def add_forecasts(self,data, model_type = "macro"):
        """
        ...
        if not data.empty:
            ...
            # Handle the case where the data is empty, possibly returning a default value or raising an informative error.
            print("len of data",len(data))
            print("data",data)
            raise ValueError("Input data is empty, unable to add forecasts")
    ...
    #                         model = ExponentialSmoothing(transformed_series, trend='add', seasonal=None)
                    ...
                    if indicator == 'volatility' or indicator == '5_min_volatility':
                        ...
                        model_fit = model.fit()
                        # print(f"steps in datahandler{steps}")
                        forecast_values = model_fit.forecast(steps=steps)
    #                         forecast_values = np.exp(forecast_values) - 1 - shift
    #                         forecast_values[forecast_values < 0] = 0  # Ensure non-negative values
