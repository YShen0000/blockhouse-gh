...
class CustomTradingEnvironment(gym.Env):
    ...
    def __init__(self, data, scenario, action_space,original_action_space, preferred_timeframe=390, target_shares=10):
        ...
        holidays = holiday_calendar.holidays(start='2023-01-01', end='2034-12-31').to_pydatetime()
        holiday_dates = set(holiday.date() for holiday in holidays)
        market_open_time = self.timestamp.replace(hour=13, minute=30, second=0, microsecond=0)
        market_close_time = self.timestamp.replace(hour=20, minute=0, second=0, microsecond=0)

        # Check if the timestamp is within the trading hours
        if self.timestamp < market_open_time:
            # If it's before market open, set it to today's market open
            self.timestamp = market_open_time
        elif self.timestamp >= market_close_time:
            # If it's after market close, find the next valid trading day
            print("Timestamp is outside trading hours, adjusting to the next market open.")

            # Start from the next day
            next_day = self.timestamp + timedelta(days=1)

            # Find the next valid trading day (skip weekends and U.S. holidays)
            while True:
                # Set the next day’s market open time
                next_market_open = next_day.replace(hour=13, minute=30, second=0, microsecond=0)

                # Check if the next day is a weekday and not a holiday
                if next_market_open.weekday() < 5 and next_market_open.date() not in holiday_dates:
                    # If it's a weekday and not a holiday, set the timestamp to the next market open
                    self.timestamp = next_market_open
                    break
                else:
    ...
    def reset(self):
        ...
        # self.timestamp = self.verify_and_correct_timestamp(self.data['timestamp'].iloc[0])
        # print("state",state)
        return state
    
    def verify_and_correct_timestamp(timestamp):
        """
        Verifies and corrects the timestamp format and timezone information.
        Args:
        - timestamp (pd.Timestamp): The timestamp to check and correct.
        Returns:
        - pd.Timestamp: The corrected timestamp in UTC.
        """
    ...
    def _unscale_action(self, scaled_action: np.ndarray) -> np.ndarray:
        ...
        """
         # Unscale the action from [-1, 1] to [low, high]
        unscaled_action = self.low + 0.5 * (scaled_action + 1) * (self.high - self.low)

        # Apply constraints to ensure the share percentage remains within defined limits
        # unscaled_action[0] = max(0.17, min(unscaled_action[0], 0.33))  # Ensure between 17% and 33%
...
#         # # Enforce the minimum share percentage to be at least 17%
#         if unscaled_action[0] < 0.17:  # Check if share percentage is less than 17%
#             unscaled_action[0] = 0.17  # Set to minimum 17%
#         if unscaled_action[0] > 0.33:  # Check if share percentage is less than 17%
#             unscaled_action[0] = 0.33
...
#         return unscaled_action
    ...
    def step(self, action):
        ...
        action = self._unscale_action(action)
                # Step counter for scaling adjustments
        self.step_counter += 1  # Increment at each step to adjust scaling

        # Introduce a scaling factor that adjusts based on step count or elapsed time
        scaling_factor = 1 + (self.step_counter / self.preferred_timeframe) * 0.2  # Scale up slightly as time progresses

        # Adjust the size of the slice by scaling factor
        if  self.step_counter ==1:
            size_of_slice  = action[0] *self.target_shares *0.5
        
        elif  self.step_counter > 4:
            size_of_slice  = action[0] *self.prev_slice 
            self.prev_slice = 0.9 * self.prev_slice
        else:
            size_of_slice = action[0] * (self.target_shares - self.shares_bought) * scaling_factor
            size_of_slice = int(np.ceil(size_of_slice))
            self.prev_slice  = (self.target_shares - self.shares_bought)
        
        # self.timestamp = self.verify_and_correct_timestamp(self.data['timestamp'].iloc[step])
        ...
        # # min_slice_size = 0.17 * (self.target_shares - self.shares_bought)
        # # if size_of_slice < min_slice_size:
        #     size_of_slice = min_slice_size
        size_of_slice = int(np.ceil(size_of_slice))
        ...
        if self.timestamp.tzinfo is None:
            # Assuming self.timestamp is naive (i.e., no timezone info), make it UTC aware.
            timestamp_utc = utc_timezone.localize(self.timestamp)
        else:
        ...
        trade_info = {
            'timestamp': timestamp_est,#self.timestamp,
            'action': action,
...
class TradingEnvironmentMacroV2(gym.Env):
    ...
    def _calculate_reward(self, size_of_slice, execution_price, timing_of_slice):
        ...
        api_key = 'r65B9O5aplSJn7BWSo8z8pNH8v2wW5yc'#'db-s8TQsSX8JF539yQSeBWFGPNyDx4m3'
...
class TradingEnvironmentMicro(Env):
    ...
    def get_expected_price(self, symbol, timestamp_str):
        ...
        try:
            ...
            # timestamp_utc = timestamp.tz_localize('America/New_York').tz_convert('UTC')
        except Exception as e:
        ...
        # Make the API call to fetch quotes from Polygon
        try:
        ...
        if max_bid_price == float('-inf'):
            print(f"No bid prices found for the given timestamp: {timestamp_str}")
            return None, None, None, None, None

        # The expected price is the maximum bid price as per the new logic
        expected_price = max_bid_price

        return expected_price, bid_sizes, ask_sizes, bid_prices, ask_prices
    ...
    def get_best_mid_price(self, data, action_type="buy"):
        """
        Calculates the best mid-price for buying or selling.

        Args:
        - data (pd.DataFrame): Data containing 'high' and 'low' prices.
        - action_type (str): Either 'buy' or 'sell' to determine whether to get the best price for buying or selling.

        Returns:
        - best_mid_price (float): The best mid-price based on the action type.
        - timestamp (pd.Timestamp): Timestamp of the best price.
        """
        if action_type == "buy":
            # For buying, you want the lowest price, so choose the lowest high and low
            best_low = data['low'].min()  # Lowest low price in the period
            best_high = data['high'].min()  # Lowest high price in the period
            best_mid_price = (best_low + best_high) / 2  # Best mid-price for buying
            # best_price_row = data.loc[(data['low'] == best_low) & (data['high'] == best_high)]

        elif action_type == "sell":
            # For selling, you want the highest price, so choose the highest high and low
            best_low = data['low'].max()  # Highest low price in the period
            best_high = data['high'].max()  # Highest high price in the period
            best_mid_price = (best_low + best_high) / 2  # Best mid-price for selling
            # best_price_row = data.loc[(data['low'] == best_low) & (data['high'] == best_high)]

        # Get the corresponding timestamp for the best price
        # timestamp = best_price_row['timestamp'].iloc[0]
      
        return best_mid_price
    def step(self, action):
        ...
        try:
            timestamp = pd.to_datetime(timestamp_str)
            timestamp_str_utc = timestamp.strftime('%Y-%m-%d %H:%M:%S')
        except Exception as e:
            print(f"Error parsing timestamp: {e}")
        ...
        cancel_actions = action[2:2 + self.max_orders]

        volume = self.data['shares'].iloc[self.current_step]
        market_price = self.data['close'].iloc[self.current_step]
        mid_price = (self.data['open'].iloc[self.current_step] + self.data['close'].iloc[self.current_step]) / 2
        limit_price = mid_price - limit_price_scale * (0.005 * mid_price)
        ...
        # mid_price = (self.data['high'].iloc[self.current_step] + self.data['low'].iloc[self.current_step]) / 2
        mid_pricings = self.get_best_mid_price(self.data, action_type="buy")
        print("mid pricings", mid_pricings)
        
        # New mid-price using high and low prices
        trade_info = {
            'step': self.current_step,
            'timestamp': timestamp_str_utc,
            'order_type': 'Market' if order_type < 0.5 else 'Limit',
            'volume': volume,
            'limit_price': mid_pricings
        }
        self.current_step += 1
        self.results.append(trade_info)

        # Avoid printing and resetting here to accumulate all trades
...
#         # print("type:", type(timestamp_str))

#         # Directly parse the timestamp without converting to UTC
#         try:
#             timestamp = pd.to_datetime(timestamp_str)  # Assuming the timestamp is already in UTC format
#             timestamp_str_utc = timestamp.strftime('%Y-%m-%d %H:%M:%S')  # Convert to string for consistency
#         except Exception as e:
#             print(f"Error parsing timestamp: {e}")
#             timestamp_str_utc = timestamp_str  # Fallback to the original if parsing fails

#         expected_price = self.data['expected_price'].iloc[self.current_step]

#         # Define order type and other action components
...
#             })
#         else:
#             # Manage live orders if not immediately executed
#             if not any(order['price'] == limit_price for order in self.live_orders):
#                 self.live_orders.append({'volume': volume, 'price': limit_price, 'time_active': 0})

#         # Apply cancellations and update live orders
#         new_live_orders = []
#         for i, order in enumerate(self.live_orders[:len(cancel_actions)]):
#             if cancel_actions[i] > 0.5:
...
#         # Check for completed trade set and reset if necessary
...
#         return self._get_state(), reward, done, {}
    ...
    def print_trades(self):
        ...
        if self.results:
            trades_df = pd.DataFrame(self.results)
            print(f'Trade Set Completed: {self.data["Trade_Set_ID"].iloc[0]}')  # Adjust to print a single heading
            print(trades_df.to_string(index=False))
            # Do not reset results here to maintain all trades together until the very end
