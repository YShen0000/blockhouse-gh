...
import pytz
from blockhouse_mls.equities.buy.utils.macro_model import PPO_MODEL_BEST_PARAMS, MetaLearner
from blockhouse_mls.equities.buy.utils.macro_model import CustomTransformerPolicy, MetaLearner, PPO_MODEL_BEST_PARAMS, PPO_MODEL_BEST_PARAMS_TAB, TabTransformerPolicy
from blockhouse_mls.equities.buy.utils.env import CustomTradingEnvironment, TradingEnvironmentMacroV2
from blockhouse_mls.equities.buy.utils.data_handler import InferenceDataHandler
...
class MacroTraderModel:
    ...
    def _get_small_medium(self, data, file_path, market_cap='large', get_tab_transformer = False, training_config=PPO_MODEL_BEST_PARAMS):
        ...
        original_action_space = spaces.Box(low=np.array([0.17, 20]), high=np.array([0.66, 30]), dtype=np.float32)
        action_space = spaces.Box(low=np.array([-1, -1]), high=np.array([1, 1]), dtype=np.float32)
        # env = CustomTradingEnvironment(data, scenario, action_space,original_action_space, preferred_timeframe=preferred_timeframe, target_shares=target_shares)
        env = TradingEnvironmentMacroV2(data, action_space, original_action_space, preferred_timeframe=preferred_timeframe,
    ...
    def _get_medium(self, data, file_path, market_cap='large', get_tab_transformer = False, training_config=PPO_MODEL_BEST_PARAMS):
        ...
        preferred_timeframe = 200
        target_shares = 500
        original_action_space = spaces.Box(low=np.array([0.17, 30]), high=np.array([0.50, 50]), dtype=np.float32)
        action_space = spaces.Box(low=np.array([-1, -1]), high=np.array([1, 1]), dtype=np.float32)
        scenario = 'medium'
        env =TradingEnvironmentMacroV2(data,
                                        action_space,original_action_space,preferred_timeframe=preferred_timeframe,
    ...
    def _get_medium_large(self, data, file_path, market_cap='large', get_tab_transformer = False, training_config=PPO_MODEL_BEST_PARAMS):
        ...
        preferred_timeframe = 200
        target_shares = 2000
        original_action_space = spaces.Box(low=np.array([0.17, 30]), high=np.array([0.40, 50]), dtype=np.float32)
        action_space = spaces.Box(low=np.array([-1, -1]), high=np.array([1, 1]), dtype=np.float32)
...
#         return 
    ...
    def infer_macro(self, timeframe, transaction_size, market_cap_int, input_data, data, meta=MetaLearner(), inference_data_handler=InferenceDataHandler()):
        ...
        # Determine action space based on the scenario
        if scenario == 'small':
            original_action_space = spaces.Box(low=np.array([0.17, 30]), high=np.array([0.33, 50]), dtype=np.float32)
        elif scenario == 'small-medium':
            original_action_space = spaces.Box(low=np.array([0.20, 20]), high=np.array([0.66, 30]), dtype=np.float32)
        elif scenario == 'medium':
            ...
            original_action_space = spaces.Box(low=np.array([0.17, 30]), high=np.array([0.33, 50]), dtype=np.float32)
        action_space = spaces.Box(low=np.array([-1, -1]), high=np.array([1, 1]), dtype=np.float32)
        model, env = self.get_model(input_data, market_cap=market_cap, scenario=scenario)
        with torch.no_grad():
            ...
            fallback_values = {
                'open': 254.5, 'high': 256.7, 'low': 253.8, 'close': 255.6, 'volume': 4000000, 'RSI': 51.8, 'MACD': 2.4,
                'MACD_signal': 2.2, 'MACD_hist': 0.2, 'Stoch_k': -1.75, 'Stoch_d': -3.67, 'OBV': 28067000, 'Upper_BB': 270.3,
                'Middle_BB': 267.9, 'Lower_BB': 265.7, 'ATR_1': 9.84, 'ADX': 3.64, '+DI': 6.87, '-DI': 12.9, 'CCI': 13.6,
                'transaction_cost': 0.68, 'forecast_6Hr_open': 257.5, 'forecast_6Hr_close': 257.2, 'forecast_6Hr_high': 256.5,
                'forecast_6Hr_low': 255.6, 'forecast_6Hr_volatility': 0.22, 'forecast_6Hr_volume': 9037326,
                'forecast_6Hr_transaction_cost': 0.68
            }
...
#                                         'forecast_6Hr_volume', 'forecast_6Hr_transaction_cost']
#             fallback_values = {
#                                 'open': 254.5, 'high': 256.7, 'low': 253.8, 'close': 255.6,
#                                 'volume': 4000000, 'RSI': 51.8, 'MACD': 2.4, 'MACD_signal': 2.2, 'MACD_hist': 0.2,
#                                 'Stoch_k': -1.75, 'Stoch_d': -3.67, 'OBV': 28067000, 'Upper_BB': 270.3, 'Middle_BB': 267.9,
#                                 'Lower_BB': 265.7, 'ATR_1': 9.84, 'ADX': 3.64, '+DI': 6.87, '-DI': 12.9, 'CCI': 13.6,
#                                 'transaction_cost': 0.68, 'forecast_6Hr_open': 257.5, 'forecast_6Hr_close': 257.2,
#                                 'forecast_6Hr_high': 256.5, 'forecast_6Hr_low': 255.6, 'forecast_6Hr_volatility': 0.22,
#                                 'forecast_6Hr_volume': 9037326, 'forecast_6Hr_transaction_cost': 0.68
