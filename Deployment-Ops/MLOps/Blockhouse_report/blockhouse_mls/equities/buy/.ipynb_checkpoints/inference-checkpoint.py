...
from blockhouse_mls.equities.buy.utils.data_handler import DataProcessor, InferenceDataHandler
from blockhouse_mls.equities.buy.utils.macro_model import MetaLearner
from blockhouse_mls.equities.buy.utils.macro_model_utils import MacroTraderModel
from blockhouse_mls.equities.buy.utils.fetch_merge_data import PolygonClient
from blockhouse_mls.equities.buy.utils.env import  TradingEnvironmentMicro #TradingEnvironment, CustomTradingEnvironment,
from blockhouse_mls.equities.buy.utils.micro_model_utils import MicroTraderModel
...
class EquitiesBuyInference:
    ...
    def run_pipeline(self, ticker, start_timestamp = (datetime.now(pytz.UTC) - timedelta(days=2)).strftime('%Y-%m-%d'), end_timestamp=datetime.now(pytz.UTC).strftime('%Y-%m-%d'), timeframe=390, inventory=10000, trade_set_counter=1):
        ...
        print("data in inference scripts", len(data))
        print("data onyl in inference script", data)
        data = self.data_processor.add_technical_indicators(data)
        print("processed data in inference scripts", len(data))
        print("processed data onyl in inference script", data)
