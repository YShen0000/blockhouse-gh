...
def model_fn(model_dir):
    ...
    logger.info(f"NVIDIA Driver Version: {get_nvidia_driver_version()}")
    # Define S3 bucket
    bucket_name = 'sagemaker-tradingmodel-us-east-1'

    # Define the paths for sell models (macro_trader and micro_trader)
    sell_macro_prefix = 'production/version_1/equities/sell/macro_trader/'
    sell_micro_prefix = 'production/version_1/equities/sell/micro_trader/'
    
    # Define the paths for buy models (macro_trader and micro_trader)
    buy_macro_prefix = 'production/version_1/equities/buy/macro_trader/'
    buy_micro_prefix = 'production/version_1/equities/buy/micro_trader/'
...
def run_equities_inference(ticker, action, model):
    """Run the actual equities inference for buy or sell models."""
    utc = pytz.UTC
    timeframe = 390
    inventory = 1

    end_timestamp = datetime.now(utc) - timedelta(days=2)
    end_timestamp = adjust_for_weekends_and_holidays(end_timestamp)

    start_timestamp = end_timestamp - timedelta(days=2)
    start_timestamp = adjust_for_weekends_and_holidays(start_timestamp)

    start_timestamp_str = start_timestamp.strftime('%Y-%m-%d')
    end_timestamp_str = end_timestamp.strftime('%Y-%m-%d')

    logger.info(f"Running {action} inference from {start_timestamp_str} to {end_timestamp_str}")
